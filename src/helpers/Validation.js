...
const INVALIDVALUE = {
    name: 'The name must contain only the letters a-z, A-C and a space, with length from 3 to 20 characters',
    number: 'The number must contain only integers from 0-9, 10 characters long',
...
class Validation {
    ...
    checkUnique = (contacts, contact) => {
        return contacts.some(savedContact => savedContact.name.toLowerCase() === contact.name.toLowerCase());
    };
}
